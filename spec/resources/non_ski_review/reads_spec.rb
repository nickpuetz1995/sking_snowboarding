require "rails_helper"

RSpec.describe NonSkiReviewResource, type: :resource do
  describe "serialization" do
    let!(:non_ski_review) { create(:non_ski_review) }

    it "works" do
      render
      data = jsonapi_data[0]
      expect(data.id).to eq(non_ski_review.id)
      expect(data.jsonapi_type).to eq("non_ski_reviews")
    end
  end

  describe "filtering" do
    let!(:non_ski_review1) { create(:non_ski_review) }
    let!(:non_ski_review2) { create(:non_ski_review) }

    context "by id" do
      before do
        params[:filter] = { id: { eq: non_ski_review2.id } }
      end

      it "works" do
        render
        expect(d.map(&:id)).to eq([non_ski_review2.id])
      end
    end
  end

  describe "sorting" do
    describe "by id" do
      let!(:non_ski_review1) { create(:non_ski_review) }
      let!(:non_ski_review2) { create(:non_ski_review) }

      context "when ascending" do
        before do
          params[:sort] = "id"
        end

        it "works" do
          render
          expect(d.map(&:id)).to eq([
                                      non_ski_review1.id,
                                      non_ski_review2.id,
                                    ])
        end
      end

      context "when descending" do
        before do
          params[:sort] = "-id"
        end

        it "works" do
          render
          expect(d.map(&:id)).to eq([
                                      non_ski_review2.id,
                                      non_ski_review1.id,
                                    ])
        end
      end
    end
  end

  describe "sideloading" do
    # ... your tests ...
  end
end
